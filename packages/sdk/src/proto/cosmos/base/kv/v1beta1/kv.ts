// @ts-nocheck
// @generated by protobuf-ts 2.5.0 with parameter long_type_string
// @generated from protobuf file "cosmos/base/kv/v1beta1/kv.proto" (package "cosmos.base.kv.v1beta1", syntax proto3)
// tslint:disable
import type { BinaryWriteOptions } from '@protobuf-ts/runtime'
import type { IBinaryWriter } from '@protobuf-ts/runtime'
import { WireType } from '@protobuf-ts/runtime'
import type { BinaryReadOptions } from '@protobuf-ts/runtime'
import type { IBinaryReader } from '@protobuf-ts/runtime'
import { UnknownFieldHandler } from '@protobuf-ts/runtime'
import type { PartialMessage } from '@protobuf-ts/runtime'
import { reflectionMergePartial } from '@protobuf-ts/runtime'
import { MESSAGE_TYPE } from '@protobuf-ts/runtime'
import { MessageType } from '@protobuf-ts/runtime'
/**
 * Pairs defines a repeated slice of Pair objects.
 *
 * @generated from protobuf messages cosmos.base.kv.v1beta1.Pairs
 */
export interface Pairs {
  /**
   * @generated from protobuf field: repeated cosmos.base.kv.v1beta1.Pair pairs = 1;
   */
  pairs: Pair[]
}
/**
 * Pair defines a key/value bytes tuple.
 *
 * @generated from protobuf messages cosmos.base.kv.v1beta1.Pair
 */
export interface Pair {
  /**
   * @generated from protobuf field: bytes key = 1;
   */
  key: Uint8Array
  /**
   * @generated from protobuf field: bytes value = 2;
   */
  value: Uint8Array
}
// @generated messages type with reflection information, may provide speed optimized methods
class Pairs$Type extends MessageType<Pairs> {
  constructor() {
    super('cosmos.base.kv.v1beta1.Pairs', [
      {
        no: 1,
        name: 'pairs',
        kind: 'message',
        repeat: 1 /*RepeatType.PACKED*/,
        T: () => Pair,
        options: { 'gogoproto.nullable': false },
      },
    ])
  }
  create(value?: PartialMessage<Pairs>): Pairs {
    const message = { pairs: [] }
    globalThis.Object.defineProperty(message, MESSAGE_TYPE, {
      enumerable: false,
      value: this,
    })
    if (value !== undefined) reflectionMergePartial<Pairs>(this, message, value)
    return message
  }
  internalBinaryRead(
    reader: IBinaryReader,
    length: number,
    options: BinaryReadOptions,
    target?: Pairs,
  ): Pairs {
    let message = target ?? this.create(),
      end = reader.pos + length
    while (reader.pos < end) {
      let [fieldNo, wireType] = reader.tag()
      switch (fieldNo) {
        case /* repeated cosmos.base.kv.v1beta1.Pair pairs */ 1:
          message.pairs.push(
            Pair.internalBinaryRead(reader, reader.uint32(), options),
          )
          break
        default:
          let u = options.readUnknownField
          if (u === 'throw')
            throw new globalThis.Error(
              `Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`,
            )
          let d = reader.skip(wireType)
          if (u !== false)
            (u === true ? UnknownFieldHandler.onRead : u)(
              this.typeName,
              message,
              fieldNo,
              wireType,
              d,
            )
      }
    }
    return message
  }
  internalBinaryWrite(
    message: Pairs,
    writer: IBinaryWriter,
    options: BinaryWriteOptions,
  ): IBinaryWriter {
    /* repeated cosmos.base.kv.v1beta1.Pair pairs = 1; */
    for (let i = 0; i < message.pairs.length; i++)
      Pair.internalBinaryWrite(
        message.pairs[i],
        writer.tag(1, WireType.LengthDelimited).fork(),
        options,
      ).join()
    let u = options.writeUnknownFields
    if (u !== false)
      (u == true ? UnknownFieldHandler.onWrite : u)(
        this.typeName,
        message,
        writer,
      )
    return writer
  }
}
/**
 * @generated MessageType for protobuf messages cosmos.base.kv.v1beta1.Pairs
 */
export const Pairs = new Pairs$Type()
// @generated messages type with reflection information, may provide speed optimized methods
class Pair$Type extends MessageType<Pair> {
  constructor() {
    super('cosmos.base.kv.v1beta1.Pair', [
      { no: 1, name: 'key', kind: 'scalar', T: 12 /*ScalarType.BYTES*/ },
      { no: 2, name: 'value', kind: 'scalar', T: 12 /*ScalarType.BYTES*/ },
    ])
  }
  create(value?: PartialMessage<Pair>): Pair {
    const message = { key: new Uint8Array(0), value: new Uint8Array(0) }
    globalThis.Object.defineProperty(message, MESSAGE_TYPE, {
      enumerable: false,
      value: this,
    })
    if (value !== undefined) reflectionMergePartial<Pair>(this, message, value)
    return message
  }
  internalBinaryRead(
    reader: IBinaryReader,
    length: number,
    options: BinaryReadOptions,
    target?: Pair,
  ): Pair {
    let message = target ?? this.create(),
      end = reader.pos + length
    while (reader.pos < end) {
      let [fieldNo, wireType] = reader.tag()
      switch (fieldNo) {
        case /* bytes key */ 1:
          message.key = reader.bytes()
          break
        case /* bytes value */ 2:
          message.value = reader.bytes()
          break
        default:
          let u = options.readUnknownField
          if (u === 'throw')
            throw new globalThis.Error(
              `Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`,
            )
          let d = reader.skip(wireType)
          if (u !== false)
            (u === true ? UnknownFieldHandler.onRead : u)(
              this.typeName,
              message,
              fieldNo,
              wireType,
              d,
            )
      }
    }
    return message
  }
  internalBinaryWrite(
    message: Pair,
    writer: IBinaryWriter,
    options: BinaryWriteOptions,
  ): IBinaryWriter {
    /* bytes key = 1; */
    if (message.key.length)
      writer.tag(1, WireType.LengthDelimited).bytes(message.key)
    /* bytes value = 2; */
    if (message.value.length)
      writer.tag(2, WireType.LengthDelimited).bytes(message.value)
    let u = options.writeUnknownFields
    if (u !== false)
      (u == true ? UnknownFieldHandler.onWrite : u)(
        this.typeName,
        message,
        writer,
      )
    return writer
  }
}
/**
 * @generated MessageType for protobuf messages cosmos.base.kv.v1beta1.Pair
 */
export const Pair = new Pair$Type()
